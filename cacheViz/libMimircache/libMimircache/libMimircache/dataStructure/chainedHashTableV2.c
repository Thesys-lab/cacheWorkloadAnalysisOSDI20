...
static inline void add_to_bucket(hashtable_t *hashtable, cache_obj_t *cache_obj) {
...
void free_chained_hashtable_v2(hashtable_t *hashtable) {
  ...
  my_free(sizeof(cache_obj_t *) * hashsize(hashtable->hash_power), hashtable->ptr_table);
...
void _chained_hashtable_expand_v2(hashtable_t *hashtable) {
  ...
  DEBUG2("hashtable resized from %llu to %llu\n", hashsizeULL((uint16_t) (hashtable->hash_power - 1)),
         hashsizeULL(hashtable->hash_power));

  cache_obj_t *cur_obj, *next_obj;
  for (uint64_t i = 0; i < hashsize((uint16_t) (hashtable->hash_power - 1)); i++) {
    cur_obj = old_table[i];
    while (cur_obj != NULL) {
      next_obj = cur_obj->hash_next;
...
void check_hashtable_integrity_v2(hashtable_t *hashtable) {
  ...
  for (uint64_t i = 0; i < hashsize(hashtable->hash_power); i++) {
    ...
    while (cur_obj != NULL) {
      next_obj = cur_obj->hash_next;
      assert(i == (get_hash_value_int_64(&cur_obj->obj_id_int) & hashmask(hashtable->hash_power)));
      cur_obj = next_obj;
    }
  }
}
